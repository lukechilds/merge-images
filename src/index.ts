...
const normalizeSource = (source: ImageInput): ImageSourceOptions => {
	if ((source as {constructor: {name: string}}).constructor.name !== 'Object') {
		return {
...
const mergeImages = (sources: ImageInput[] = [], options: MergeImagesOptions = {}): Promise<string> => new Promise(resolve => {
	...
	const canvas = (resolvedOptions.Canvas ? new resolvedOptions.Canvas() : globalThis.document.createElement('canvas')) as CanvasLike;
	const ImageConstructor = (resolvedOptions.Image || globalThis.Image) as ImageConstructor;

	// Load sources.
	const images = sources.map(source => new Promise<LoadedImage>((resolve, reject) => {
		...
		.then(images => {
			...
			if (resolvedOptions.Canvas && resolvedOptions.format === 'image/jpeg') {
				...
				return new Promise<string>((resolve, reject) => {
					canvas.toDataURL(resolvedOptions.format, {
